...
// id -> primary muscle group, for the per-muscle fallback
const muscleById = new Map(
  [...ours.matchAll(/id: '([a-z0-9-]+)',[^\n]*?muscle: '([a-z]+)'/g)].map((m) => [m[1], m[2]])
)

// A movement's "base" key ignores the equipment qualifier in parens and a few
// intensity prefixes, so "Incline Press (Smith Machine)" and "Paused Incline
// Press" both fold onto "incline press".
const PREFIXES = ['paused', 'pause', 'pin', 'deficit', 'tempo', 'heavy', 'reverse-grip', 'close-grip', 'wide-grip', 'single-arm', 'single-leg', 'b-stance', 'iso-lateral', 'unilateral', 'seated', 'standing', 'lying', 'incline', 'decline', 'flat']
function baseKey(name) {
  let n = normalize(name.replace(/\(.*$/, '')) // drop " (Equipment...)"
  const toks = n.split(' ').filter(Boolean)
  while (toks.length > 1 && PREFIXES.includes(toks[0])) toks.shift()
  return toks.join(' ')
}
...
const directCount = Object.keys(mapping).length

// ---- Fallback stage 1: reuse a base-movement image for variations ----
const baseImage = new Map() // baseKey -> url (prefer the shortest source name = most canonical)
const baseNameLen = new Map()
for (const { id, name } of entries) {
  if (!mapping[id]) continue
  const key = baseKey(name)
  if (!baseImage.has(key) || name.length < baseNameLen.get(key)) {
    baseImage.set(key, mapping[id])
    baseNameLen.set(key, name.length)
  }
}

// ---- Fallback stage 2: per-muscle representative image ----
const muscleImage = new Map() // muscle -> url
for (const { id, name } of entries) {
  if (!mapping[id]) continue
  const mus = muscleById.get(id)
  if (mus && !muscleImage.has(mus)) muscleImage.set(mus, mapping[id])
}
const anyImage = Object.values(mapping)[0]

let borrowedBase = 0
let borrowedMuscle = 0
const stillMissing = []
for (const { id, name } of unmatched) {
  const viaBase = baseImage.get(baseKey(name))
  if (viaBase) {
    mapping[id] = viaBase
    borrowedBase++
    continue
  }
  const viaMuscle = muscleImage.get(muscleById.get(id))
  if (viaMuscle) {
    mapping[id] = viaMuscle
    borrowedMuscle++
    continue
  }
  mapping[id] = anyImage // absolute backstop — never leave one blank
  stillMissing.push(`${id} (${name})`)
}

console.log('direct matches:', directCount)
console.log('borrowed base-movement image:', borrowedBase)
console.log('borrowed per-muscle image:', borrowedMuscle)
console.log('generic backstop:', stillMissing.length, stillMissing)
console.log('TOTAL with image:', Object.keys(mapping).length, '/', entries.length)
